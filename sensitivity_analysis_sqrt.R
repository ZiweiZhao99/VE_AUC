...
lambda.t <- function(t) 0.15*exp(beta+(beta0+beta1*sqrt(t))*z)
...
# function to generate recurrent event times
lambda.t <- function(t) (0.1*(t<6)+0.2*(t>=6))*exp(beta+(beta0+beta1*sqrt(t))*z)
lambda.i <- function(t) (0.1*(t<6)+0.2*(t>=6))*exp(beta+(beta0+beta1*sqrt(t))) #hazard of intervention arm
...
# function to generate recurrent event times
lambda.t <- function(t) (0.2*(t<6)+0.1*(t>=6))*exp(beta+(beta0+beta1*sqrt(t))*z)
lambda.i <- function(t) (0.2*(t<6)+0.1*(t>=6))*exp(beta+(beta0+beta1*sqrt(t))) #hazard of intervention arm
...
NUM_REPS <- 500 # however many replications you want

plan(multisession, workers = 25)  # choose number of cores

List_sqrt <- future_map(
  1:NUM_REPS,
  one_simulation,
  .progress = TRUE,
...
save(List_sqrt, file = "./500 sensitivity sqrt.RData")
